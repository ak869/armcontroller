...
static    OS_EVENT	* ReadNoEmpty;        /* 读空处理函数             */
typedef struct {
    uint8     *Out;                   /* 指向数据输出位置         */
    uint8     *In;                    /* 指向数据输入位置         */
...
uint8 NMsgQWrite( struct tag_nodemsg *Data, void *Buf)
{
	uint8 err;
	NMsgDataQueue *Queue;
	uint8 *tmp,nodeSize;
